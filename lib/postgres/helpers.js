...
helpers.DB_CRIDENTIALS = {
    ...
    user: 'test1234',
    password: 'test1234',
    database: 'postgres', // It's a master database
};
...
helpers.create = (cridentials, dbName, cb)=>{
    ...
    const cridentials2 = JSON.parse(JSON.stringify(cridentials));
    cridentials2.database = helpers.DB_CRIDENTIALS.database;  // Override database if to PG master database

    Connection.create(cridentials2, undefined, (err, dbc)=>{
        if (err) {
            cb(err);
        } else {
            assert(dbc);
