...
helpers.list = (cb)=> {
    ...
    helpers.connectAndQuery(helpers.DB_CRIDENTIALS.database, q, (err, dbc, result)=>{
        if(dbc) {
            dbc.close(()=>{
                cb(err, result);
            });
...
//
helpers.createAndConnect = (dbName, cb)=>{
    assert(dbName);
    assert(cb);
...
helpers.delete = (dbName, cb)=>{
    ...
    helpers.connectAndQuery(helpers.DB_CRIDENTIALS.database, queries[0], (err, dbc, result)=>{
        if(dbc) {
            dbc.close(()=>{
                cb(err, result);
            });
