...
describe('Db', ()=>{
    ...
    describe('#Db.open, #Db.close', ()=>{
        ...
        it('Create databases', (done)=>{
            ...
            databases = {};
            databases.db1 = JSON.parse(JSON.stringify(api.mysql.helpers.cridentials));
            databases.db1.schema = 'mysql';
            databases.db1.database = createDbName('mysql');
            databases.db2 =  JSON.parse(JSON.stringify(api.postgres.helpers.cridentials));
            databases.db2.schema = 'postgres';
            databases.db2.database = createDbName('postgres');
            ...
            foreach(0, entries, (d, complete)=>{
                ...
                api[dbProps.schema].helpers.create(dbProps.database, (err)=>{
                    assert(!err);
                    complete();
                });
            }, done);
