...
module.exports = (testName, api, dbHelpers, Dbc, cridentials, dbTable)=>{
    ...
    assert(cridentials);
    assert(dbTable);
    const createDbName = (name)=>{ return api.db.Db.createDbName('yeap_db_' + name); }
    const getRandomInt = (min, max)=>{ return Math.floor(Math.random() * (max - min) + min); }
    const MASTER_PERIOD = 1000;
    const WORKER_PERIOD = 1000;
    const WORKER_REQ_COUNT = 1;
    ...
    //      api.lib.log.debug('Worker.start()');
            ...
            this.timerId = setInterval(
                ()=>{
                    for(let k=0; k<WORKER_REQ_COUNT; ++k) {
    ...
    const Master = function(dbc) {
        ...
        this.handleDbSlow = function(err, res, fields, query, executionTime) {
            console.log('!!!!!! DB is slow(' + this.dbc.slowsRequestsCount + '), ' + executionTime + 'ms\t' + query);
        ...
        this.handleDbError = function(err, res, fields, query) {
            console.log('!!!!!! DB query error:' + err + '\t' + query);
        }
        ...
        this.stop = function(cb) {
            ...
            this.timerId = setInterval(
                ()=>{
                    console.log('Master.stop, workers.length:' + self.workers.length + ', requests:' + self.dbc.completeRequestsCount + ', pending requests:' + self.dbc.pendingRequestsCount);
    ...
    describe('yeap_db.performance.' + testName, ()=>{
        ...
        it('Create db.', (done)=>{
            ...
            dbName = createDbName(testName);
            console.log('db name:' + dbName);
            console.log('MASTER_PERIOD(ms):' + MASTER_PERIOD);
            console.log('WORKER_PERIOD(ms):' + WORKER_PERIOD);
            console.log('WORKER_REQ_COUNT:' + WORKER_REQ_COUNT);
