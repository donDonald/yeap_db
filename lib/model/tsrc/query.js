...
module.exports = function(SQL_DRIVER, schema) {
...
describe('model.query.' + SQL_DRIVER, ()=>{
    ...
    before(()=>{
        ...
        const cridentials = api[SQL_DRIVER].helpers.cridentials;
    ...
    //                  console.log('record:', record);
        ...
        before((done)=>{
            ...
            api[SQL_DRIVER].helpers.create(
                ...
                (err)=>{
                    assert(!err, err);
                    api[SQL_DRIVER].helpers.connect(dbProps.database, (err, result)=>{
                        assert(!err, err);
                        dbc = result;
                        dbCount = api.model.count.bind(undefined, dbc, 'records');
                        dbAdd = api.model.add.bind(undefined, dbc, 'records', Record);
                        api[SQL_DRIVER].helpers.querySqls(dbc, [schema], (err)=>{
                            assert(!err, err);
                            done();
                        });
                    });
